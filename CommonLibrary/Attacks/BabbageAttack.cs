...
using CommonLibrary.Attacks.Utils;
...
public class BabbageAttack : IAttack
{
    ...
    public readonly int MaxKeyLenght;
    ...
    public string ForceDecrypt(string message)
    {
        ...
        foreach (var repeatingSubstring in allRepeatingSubstrings)
        {
            ...
            foreach (var i in allMatches.Select((_, i) => i))
            {
                if (i == 0)
                    continue;
        ...
        var allPossibleKeyLen = differencesDict
            ...
            .SelectMany(
                x => x
                    .SelectMany(y => y.GetDivisors() ?? new List<int>())
                    .Where(y => y > 1)
            )
            .Where(x => x < MaxKeyLenght)
            .GroupBy(x => x)
            .Select(x => x.Key)
            .OrderBy(x => x)
            .ToList();
    ...
    private static List<(int, string)> AnalyseFreq(int tryKeyLen, string encryptedMessage)
    {
        ...
        {
            var nthChars = new string(encryptedMessage
                .Substring(i, encryptedMessage.Length - i)
                .Where((_, cI) => cI % tryKeyLen == 0)
                .ToArray());

            var bestFit = FindBestFit(nthChars);
    ...
    private static string FindBestFit(string nthChars)
    {
        ...
        foreach (var chr in Alphabet)
        {
            ...
            var encryptedBySubKey = cipher.Decrypt(nthChars);
        ...
        IReadOnlyCollection<(int, string)> keyBestFitPos)
    {
        ...
        return possibleKeys.Select(key =>
        {
            ...
            return (EnglishFrequencyScore(currentDecrypted), key, currentdecrypted: currentDecrypted);
        }).ToList();
    }
